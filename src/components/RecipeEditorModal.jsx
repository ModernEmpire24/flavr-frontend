...
export default function RecipeEditorModal({ open, onClose, onSave }) {
  ...
  function save() {
    const recipe = {
      ...
      ingredients: ingredients.split("\n").map(s=>s.trim()).filter(Boolean).map(s=>({ item:s, amount:"" })),
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-[100] bg-black/30 backdrop-blur-sm"
      ...
      >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-sm">
            ...
            <input className="w-full rounded-xl border border-orange-200 px-3 py-2" value={title} onChange={e=>setTitle(e.target.value)} />
          </label>
          <label className="text-sm">
            <div className="text-xs text-slate-500 mb-1">Image URL</div>
            <input className="w-full rounded-xl border border-orange-200 px-3 py-2" value={image} onChange={e=>setImage(e.target.value)} placeholder="https://..." />
          </label>
          <label className="text-sm">
            <div className="text-xs text-slate-500 mb-1">Time (minutes)</div>
            <input type="number" className="w-full rounded-xl border border-orange-200 px-3 py-2" value={time} onChange={e=>setTime(e.target.value)} />
          </label>
        </div>
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-2">
          <label className="text-sm">
            ...
            <textarea className="h-40 w-full rounded-xl border border-orange-200 px-3 py-2" value={ingredients} onChange={e=>setIngredients(e.target.value)} />
          </label>
          <label className="text-sm">
            <div className="text-xs text-slate-500 mb-1">Steps (one per line)</div>
            <textarea className="h-40 w-full rounded-xl border border-orange-200 px-3 py-2" value={steps} onChange={e=>setSteps(e.target.value)} />
          </label>
        </div>

        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-xl border px-3 py-2 text-sm">Cancel</button>
          <button onClick={save} className="rounded-xl bg-gradient-to-tr from-flame-500 via-coral-500 to-rose-500 px-4 py-2 text-sm font-medium text-white shadow-soft">
            Save recipe
          </button>
        </div>
      </motion.div>
